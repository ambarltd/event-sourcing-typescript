...
} from "./domain/cookingClub/membership/query/membersByCuisine/MembersByCuisineQueryController";
import {
...
app.use('/api/v1/cooking-club/membership/projection', AmbarAuthMiddleware, (req, res, next) => {
    const controller = req.container.resolve(MembersByCuisineProjectionController);
    return controller.router(req, res, next);
});
