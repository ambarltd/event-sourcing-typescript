...
export class MembershipApplicationRepository {
    private readonly collectionName = 'CookingClub_MembersByCuisine_MembershipApplication';
    ...
    async findOneById(_id: string): Promise<MembershipApplication | null> {
        const results = await this.mongoOperator.find<MembershipApplication>(this.collectionName, { _id });
