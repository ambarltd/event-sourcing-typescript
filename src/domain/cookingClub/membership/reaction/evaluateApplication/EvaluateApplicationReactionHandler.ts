...
export class EvaluateApplicationReactionHandler extends ReactionHandler {
    ...
    async react(event: Event): Promise<void> {
        ...
        const aggregateData = await this.postgresTransactionalEventStore.findAggregate<Membership>(event.aggregateId);
        ...
        const reactionEventId = IdGenerator.generateDeterministicId(`CookingClub_Membership_ReviewedApplication:${event.eventId}`);
        if (await this.postgresTransactionalEventStore.doesEventAlreadyExist(reactionEventId)) {
            return;
        }

        const shouldApprove = event.yearsOfProfessionalExperience == 0 && event.numberOfCookingBooksRead > 0;
