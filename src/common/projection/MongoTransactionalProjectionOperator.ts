...
export class MongoTransactionalProjectionOperator {
  ...
  async commitTransaction(): Promise<void> {
    if (!this.session) {
      throw new Error(
        'Session must be active to commit transaction to MongoDB!',
      );
    }

    if (!this.session.inTransaction()) {
      throw new Error(
        'Transaction must be active to commit transaction to MongoDB!',
      );
    }
  ...
  private async operate() {
    ...
    if (!this.session.inTransaction()) {
      throw new Error(
        'Transaction must be active to read or write to MongoDB!',
      );
    }
