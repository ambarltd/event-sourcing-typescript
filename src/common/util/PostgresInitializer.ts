...
export class PostgresInitializer {
  constructor(
    ...
    @inject('eventStoreDatabaseName')
    private readonly eventStoreDatabaseName: string,
    @inject('eventStoreTable') private readonly eventStoreTable: string,
    @inject('eventStoreCreateReplicationUserWithUsername')
    private readonly replicationUsername: string,
    @inject('eventStoreCreateReplicationUserWithPassword')
    private readonly replicationPassword: string,
    @inject('eventStoreCreateReplicationPublication')
    private readonly replicationPublication: string,
  ) {}
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      await this.executeStatementIgnoreErrors(
        client,
        `
  ...
  private async executeStatementIgnoreErrors(
    client: any,
    sqlStatement: string,
  ): Promise<void> {
    try {
